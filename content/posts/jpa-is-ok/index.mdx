...
My coworker had a very similar task, but his table had surrogate keys while mine had natural keys.
...
The [Spring Boot Kotlin Guide] mentions that data classes and JPA are not a good fit and I ended up with exactly that:
...
Looking back, It really was fun doing lots of Hibernate. Starting with rather unusual looking [derived queries] that traverse part of a [composite primary key],
to custom Spring Data queries that use typed queries via the entity manager under the hood, I feel much more comfortable with Hibernate now.
